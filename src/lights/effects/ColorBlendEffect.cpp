    ...
    : Effect(id), color1String(color1), color2String(color2), speed(speed), duration(duration), elapsed(0.0f)
{
    ...
    LOG_DEBUGF_COMPONENT("ColorBlendEffect", "Created with ID %d, color1: %s, color2: %s, speed: %f, duration: %f", id, color1String.c_str(), color2String.c_str(), speed, duration);
}
...
void ColorBlendEffect::parseColorString(const String &colorString, Light &color)
{
    ...
    if (colorString.startsWith("rgb(") && colorString.endsWith(")"))
    {
        ...
        if (firstComma > 0 && secondComma > firstComma)
        {
            ...
            color = Light(r, g, b);
            LOG_DEBUGF_COMPONENT("ColorBlendEffect", "Parsed color rgb(%d,%d,%d)", r, g, b);
        }
        else
        {
            LOG_ERRORF_COMPONENT("ColorBlendEffect", "Invalid RGB format: %s", colorString.c_str());
            color = Light(255, 255, 255); // Default to white
        }
    }
    else
    {
        LOG_ERRORF_COMPONENT("ColorBlendEffect", "Unsupported color format: %s", colorString.c_str());
        color = Light(255, 255, 255); // Default to white
    }
}

...
float do_sine(float x)
{
    // return sinf(x), converting x to radians
    return sinf(x);// * 3.14159f / 180.f);
}
...
void WavePlayer::init( Light& r_Lt0, unsigned int Rows, unsigned int Cols, Light HiLt, Light LoLt )
{
    ...
    Serial.println("wvPlay.init() HiLt.r = " + String(rd) + " HiLt.g = " + String(gn) + " HiLt.b = " + String(bu));
    rd = LoLt.r; gn = LoLt.g; bu = LoLt.b;
    // std::cout << "\n LoLt.r = " << rd << " LoLt.g = " << gn << " LoLt.b = " << bu;
    Serial.println("wvPlay.init() LoLt.r = " + String(rd) + " LoLt.g = " + String(gn) + " LoLt.b = " + String(bu));
}
...
void WavePlayer::setSeriesCoeffs( float* C_rt, unsigned int n_TermsRt, float* C_lt, unsigned int n_TermsLt )
{
    ...
    if( C_Rt )
    {
        ...
        for( unsigned int k = 0; k < nTermsRt; ++k )
        {
            C_Rt[k] /= sum;
            // std::cout << "\nC_Rt[" << k << "] = " << C_Rt[k];
            // Serial.println("C_Rt[" + String(k) + "] = " + String(C_Rt[k]));
        }
    }
    ...
    if( C_Lt )
    {
        ...
        for( unsigned int k = 0; k < nTermsLt; ++k )
        {
            C_Lt[k] /= sum;
            // std::cout << "\nC_Lt[" << k << "] = " << C_Lt[k];
            Serial.println("C_Lt[" + String(k) + "] = " + String(C_Lt[k]));
        }
    }
}
...
void WavePlayer::update( float dt )
{
    ...
    // Serial.println("mvPlay.update() numLts = " + String(numLts));
    for( unsigned int n = 0; n < numLts; ++n )
    {
        ...
        if( C_Lt )
        {
            for( unsigned k = 0; k < nTermsLt; ++k )
                yLt += C_Lt[k]*do_sine( (k+1)*arg );
        }
        else
            yLt = do_sine( arg );

        float y = AmpRt*yRt + AmpLt*yLt;
        // Serial.println("mvPlay.update() yLt = " + String(yLt) + " yRt = " + String(yRt) + " y = " + String(y));
  //      // std::cout << "\nwvPlay.update() yLt = " << yLt << " yRt = " << yRt << " y = " << y;
   ...
   //     // std::cout << "\nwvPlay.update() fr = " << fr << " fg = " << fg << " fb = " << fb;
        // Serial.println("mvPlay.update() fr = " + String(fr) + " fg = " + String(fg) + " fb = " + String(fb));
